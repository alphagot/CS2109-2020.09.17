...
//	PWM_PERIOD0H = 0x04;							   
	...
	MISC = 0x80;	  		//pb6不复用IRED，pa0复用TXD，pa1复用RXD  
...
//	INTML = 0XDF;		//TIMER23中断开启
	INTMH = 0x0D;       //uart中断使能
	INTSEL0L = 0X20;	//TIMER23中断选择int0输出 
	...
	ADPCR |= 0x08;       //PB6为AN3输入
//	BANDGAP_TEMP |= 0x02; //打开温度传感器
	LDOC=0X66;	//LDO 2.4v	
...
{	
	TH1 = 0x00;
	TL1 = 0x33;		  //2400波特率
	TTCON |= 0x02;	  //T1定时周期配置有效
	TMOD = 0x00;      //T1预分频系数1
	TTCON = 0x28;     //T1使能，T1计数使能	
	TTMOD2 = 0x00;	  //T1产生波特率，16位计数
	SCON = 0x50;	  //串口允许接收,串口工作模式1
	SMOD = 0x00;
}
...
void uart()
{
	...
	ADCR = 0x23;	      //打开ADC电源，选择电源做基准,选择AN3
	delay_20us(5);
	ADCR |= 0x80;
	delay_20us(1);
	ADCR &= ~0x80;		  //启动ADC检测
	...
	IE = 0X00;
	delay_20us(1);
	ADC_VALUE1 = ADRH;	  //取ADC高8位
	delay_20us(1);
	ADC_VALUE2 = ADRL>>4;	//取ADC低4位
	delay_20us(1);
	IE = 0X85;
	ADCR = 0x00;
...
void ser_uart() interrupt 2	   //uart中断
{
    //LED_ON()	;
	if(SCON&0x01)
	{	
